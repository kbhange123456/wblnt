...
public class ItemWisePDF {

	public static ByteArrayInputStream getItemWise(List<WeighBridgeObj> wboList,String item, LocalDate startDateLD, LocalDate endDateLD) {
        ...
        try {
            ...
            String reportType = "Item Name : " + item;
            ...
            table.setWidths(new int[]{8, 7, 8, 10, 9, 10, 10, 7, 7, 7, 6, 7, 7, 7});
			...
			hcell = new PdfPCell(new Phrase(leadingVar, "City", headFontBold));
			hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			hcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			hcell.setBorder(2);
			table.addCell(hcell);
...
//            	weightChargesTotal=weightChargesTotal.add(wbo.getChgs());
            	...
            	rowBuilder(table,wbo.getEntryTime().toLocalDate().format(formatters),wbo.getId(),wbo.getVnum(),wbo.getVtype(),wbo.getPname(),wbo.getTtype(),wbo.getAddr(),wbo.getCity(),wbo.getGweight(),wbo.getTweight(),wbo.getPweight(),wbo.getNweight(),wbo.getRate(),wbo.getAmt());
            }
...
//            rowBuilder(table,"AMOUNT","0.00","MOBILE NO","",true,false,false,false);
            ...
            table2.setWidths(new int[]{20, 21, 24, 14, 7, 7, 7});
            
            rowBuilder2(table2, "COUNT : "+wboList.size(), "","","TOTAL",netWeightTotal,"",amounTotal);
...
//            document.add(address);
	...
	private static void rowBuilder2(PdfPTable table, String string1, String string2,String gap,String gap1, BigDecimal netWeightTotal, String string, BigDecimal amounTotal) {
...
//	rowBuilder(table,wbo.getEntryTime().toLocalDate().format(formatters),wbo.getId(),wbo.getVnum(),wbo.getVtype(),wbo.getPname()
//			,wbo.getTtype(),wbo.getAddr(),wbo.getCity(),wbo.getGweight(),wbo.getTweight(),wbo.getNweight(),wbo.getRate(),wbo.getAmt());
	private static void rowBuilder(PdfPTable table, String localDate, Long id, String vnum, String vtype, String pname,
			String ttype, String addr, String city, BigDecimal gweight, BigDecimal tweight, BigDecimal pweight, BigDecimal nweight, BigDecimal rate, BigDecimal amt) {
		...
		hcell = new PdfPCell(new Phrase(leadingVar,ttype, headFont));
		hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		hcell.setHorizontalAlignment(Element.ALIGN_LEFT);
		hcell.setBorder(2);
		table.addCell(hcell);
