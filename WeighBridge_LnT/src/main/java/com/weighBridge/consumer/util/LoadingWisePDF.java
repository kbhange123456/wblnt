...
public class LoadingWisePDF {

	public static ByteArrayInputStream getLoadingeWise(List<WeighBridgeObj> wboList,String loading, LocalDate startDateLD, LocalDate endDateLD) {
        ...
        BigDecimal ulbChargesTotal = BigDecimal.ZERO;
        try {
            ...
            String reportType = "Loaded By : " + loading;
            ...
            table.setWidths(new int[]{10, 12, 10, 30, 12, 14, 12, 12});
...
//			table.addCell(hcell);
//
			hcell = new PdfPCell(new Phrase(leadingVar, "Loading Charges", headFontBold));
			hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			hcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			hcell.setBorder(2);
			table.addCell(hcell);
...
//            	weightChargesTotal=weightChargesTotal.add(wbo.getChgs());
            	ulbChargesTotal=ulbChargesTotal.add(wbo.getUlbchgs());
            	rowBuilder(table,wbo.getEntryTime().toLocalDate().format(formatters),wbo.getId(),wbo.getVnum(),wbo.getPname(),wbo.getMat(),wbo.getVtype(),wbo.getUlbchgs(),wbo.getNweight());
            }
...
//            rowBuilder(table,"AMOUNT","0.00","MOBILE NO","",true,false,false,false);
            ...
            table2.setWidths(new int[]{62,14,12,12});
            
            rowBuilder2(table2, "COUNT : "+wboList.size(),"TOTAL", ulbChargesTotal, netWeightTotal);
...
//            document.add(address);
	...
	private static void rowBuilder2(PdfPTable table, String string1, String string2,BigDecimal ulbChargesTotal, BigDecimal netWeightTotal) {
		...
		hcell = new PdfPCell(new Phrase(leadingVar,ulbChargesTotal.toString(), headFont));
		hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		hcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		hcell.setBorder(Rectangle.NO_BORDER);
		table.addCell(hcell);
...
//	rowBuilder(table,wbo.getEntryTime().toLocalDate().format(formatters),
//			wbo.getId(),wbo.getVnum(),wbo.getPname(),wbo.getMat(),
//			wbo.getVtype(),wbo.getUlbchgs(),wbo.getNweight());
	private static void rowBuilder(PdfPTable table, String localDate, Long id, String vnum, String pname,
			String mat, String vtype, BigDecimal ulbchgs, BigDecimal nweight) {
...
//		table.addCell(hcell);
//		
		hcell = new PdfPCell(new Phrase(leadingVar, ulbchgs.toString(), headFont));
		hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		hcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		hcell.setBorder(2);
		table.addCell(hcell);
